...
if ($mod=='marketing' AND $act=='menuedit'){
...
// Delete User
elseif ($mod=='marketing' AND $act=='deletemarketing'){
	function anti_injection($data){
    $filter = stripslashes(strip_tags(htmlspecialchars($data,ENT_QUOTES)));
    return $filter;
	}
	...
	if($currentRoleAccess->HakAkses == "Admin" AND $currentRoleAccess->password == $passw){
		$id = $val->validasi($_POST['id'],'sql');
		$tabledel = new MyTable('tblmarketing');
		$currentdata = $tabledel->findBy('KodeMarketing',$id);
		$currentdata = $currentdata->current();
		
		$tabledel2 = new MyTable('tblrekanan');
		$tabledel2->deleteBy('IDRekanan', $currentdata->IDMarketing);
		
		$tabledel->deleteBy('KodeMarketing', $id);
...
// Input user
elseif ($mod=='perwakilan' AND $act=='input'){
	if($currentRoleAccess->HakAkses == "Admin"){
		$ids = $val->validasi($_POST['idper'],'xss');
		...
		$keterangan = $val->validasi($_POST['keterangan'],'xss');
			...
			$tblperwakilan = $tableuser->findAll('KodePerwakilan', 'ASC');
			foreach($tblperwakilan as $user){
				$user = $user->KodePerwakilan;
			}
			$KodePerwakilan = $user + 1;
			$table = new MyTable('tblperwakilan');
			$table->save(array(
				'KodePerwakilan' => $KodePerwakilan,
				'IDPerwakilan' => $ids,
				'Nama' => $nama,
				'Alamat' => $alamat,
				'NoPonsel' => $noponsel,
...
// Update user
elseif ($mod=='marketing' AND $act=='update'){
	if($currentRoleAccess->HakAkses == "Admin"){
		$id = $val->validasi($_POST['kodemarketing'],'xss');
		$ids = $val->validasi($_POST['idper'],'xss');
		$nama = $val->validasi($_POST['nama'],'xss');
		//$pass = md5($_POST[password]);
		$alamat = $val->validasi($_POST['alamat'],'xss');
		...
		$email = $val->validasi($_POST['email'],'xss');
		$tanggallahir = $val->validasi($_POST['tanggallahir'],'xss');
		$tanggaldaftar = $val->validasi($_POST['tanggaldaftar'],'xss');
		$jeniskelamin = $val->validasi($_POST['jeniskelamin'],'xss');
		$status = $val->validasi($_POST['status'],'xss');
		$keterangan = $val->validasi($_POST['keterangan'],'xss');
		$level = $val->validasi($_POST['level'],'xss');
		$idreferal = $val->validasi($_POST['idreferal'],'xss');
		$idperwakilan = $val->validasi($_POST['idperwakilan'],'xss');
		$data = array(
			'IDMarketing' => $ids,
			'Nama' => $nama,
			'Alamat' => $alamat,
		...
		$table->updateBy('KodeMarketing', $id, $data);
