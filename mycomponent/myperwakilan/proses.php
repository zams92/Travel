...
if ($mod=='perwakilan' AND $act=='menutambah'){
...
elseif ($mod=='perwakilan' AND $act=='menuedit'){
	function anti_injection($data){
    $filter = stripslashes(strip_tags(htmlspecialchars($data,ENT_QUOTES)));
    return $filter;
	}
...
// Delete User
elseif ($mod=='perwakilan' AND $act=='deleteperwakilan'){
	function anti_injection($data){
    $filter = stripslashes(strip_tags(htmlspecialchars($data,ENT_QUOTES)));
    return $filter;
	}
	...
	if($currentRoleAccess->HakAkses == "Admin" AND $passw == $currentRoleAccess->password){
		$id = $val->validasi($_POST['id'],'sql');
		$tabledel = new MyTable('tblperwakilan');
		$currentdata = $tabledel->findBy('KodePerwakilan',$id);
		$currentdata = $currentdata->current();
		
		$tabledel2 = new MyTable('tblrekanan');
		$tabledel2->deleteBy('IDRekanan', $currentdata->IDPerwakilan);
		
		$tabledel->deleteBy('KodePerwakilan', $id);
...
// Update user
elseif ($mod=='perwakilan' AND $act=='update'){
	if($currentRoleAccess->HakAkses == "Admin"){
		$id = $val->validasi($_POST['kodeperwakilan'],'xss');
		$ids = $val->validasi($_POST['idper'],'xss');
		$nama = $val->validasi($_POST['nama'],'xss');
		//$pass = md5($_POST[password]);
		$alamat = $val->validasi($_POST['alamat'],'xss');
		...
		$table = new MyTable('tblperwakilan');
		$table->updateBy('KodePerwakilan', $id, $data);
