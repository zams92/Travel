...
$aksi="mycomponent/mypendaftaran/proses.php";
?>
            ...
            <div class="col-md-12">
                    <div class="block full">
                        ...
                        <?php
                            ...
                            $users = $tableuser->findAllAnd(KodeDaftar, DESC ,Status,'N');
                            echo "<table cellpadding='0' cellspacing='0' border='0' class='table table-vcenter table-condensed table-bordered' id='table-datatable'>
                                <thead><tr>
                                    ...
                                    <th class='text-center'style='font-size: 13px;'>Telepon</th>
                                    ...
                                    <th class='text-center'style='font-size: 13px;'>Daftar Sebagai</th>
                                    <th class='text-center'style='font-size: 13px;'>ID Rekanan</th>
                                    <th class='text-center'style='font-size: 13px;'>Action</th>
                                </tr></thead>"; 
                                $no=1;
                                foreach($users as $user){
                                    $tablelevel = new MyTable('tblpendaftaran');
                                    $currentLevel = $tablelevel->findBy(KodeDaftar, $user->KodeDaftar);
                                    $currentLevel = $currentLevel->current();
									if($user->DaftarSebagai == '1'){
										$DaftarSebagai = "<span class='label label-success'>Marketing</span>";
									}elseif($user->DaftarSebagai == '2'){ 
										$DaftarSebagai = "<span class='label label-info'>Haji</span>";
									}else {
										$DaftarSebagai = "<span class='label label-info'>Umroh</span>";									
									}
									if($user->Status == 'Y'){
                                    ...
                                    echo "<tr>
                                        ...
                                        <td class='text-center'>
                                            <div class='text-center'><div class='btn-group btn-group-xs'>
												<a data-placement='left' class='enable-tooltip btn btn-xs btn-success alertdeljabatan' id='$user->KodeDaftar' title='Verifikasi'><i class='fa fa-check'></i></a>
                                                <a data-placement='right' class='enable-tooltip btn btn-xs btn-danger alertdel' id='$user->KodeDaftar' title='Delete'><i class='fa fa-times'></i></a>
                                            </div></div>
                                        </td>
                                    </tr>";
                                    $no++;
			...
			<div class="modal-content">
				<form id="form-validation" class="form-horizontal" method="post" action="<?=$aksi;?>" autocomplete="off">
					...
					<div class="modal-body">
						<input type="hidden" name="mod" value="pendaftaran">
						<input type="hidden" name="act" value="deletependaftaran">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<form id="form-validation" class="form-horizontal" method="post" action="<?=$aksi;?>" autocomplete="off">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
						<h3 id="modal-title"><i class="fa fa-info-circle text-info"> </i> Input Password to Verify </h3>
					</div>
					<div class="modal-body">
						<input type="hidden" name="mod" value="pendaftaran">
...
<?php
    ...
    if ($_SESSION[hakakses]=='Admin'){
		...
		$table1  = new MyTable('tblperwakilan');
		$total1  = $table1->numRowBy(IDPerwakilan, $string);
		}
		while (total1 > 0);
		return $string; 
		}
		      ...
		      <form id="form-validation" class="form-horizontal" method="post" action="<?=$aksi;?>" autocomplete="off">
                <fieldset>
                    ...
                    <div class="form-group">
                        <label class="col-md-4 control-label">Jenis Kelamin<span class="text-danger">*</span></label>
                        <div class="col-md-8">
                        <Select class="form-control" name="jeniskelamin" value="<?=$currentUser->JenisKelamin; ?>" >
						<?php
                    ...
                    <div class="form-group">
                        <label class="col-md-4 control-label">Status <span class="text-danger">*</span></label>
                        <div class="col-md-8">
                        <Select class="form-control" name="status"value="<?=$currentUser->Status; ?>" >
						<?php
