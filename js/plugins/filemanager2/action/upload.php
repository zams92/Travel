...
$thumbpath = preg_replace("/\/(myupload$)/","",$path);
verifyPath($path);
$res = '';
if(is_dir(fixPath($path))){
  if(!empty($_FILES['files']) && is_array($_FILES['files']['tmp_name'])){
    ...
    foreach($_FILES['files']['tmp_name'] as $k=>$v){
	  ...
	  $fileThumbPath = fixPath($thumbpath).'/../../mythumbs/'.$filename;
	  $fileMediumPath = fixPath($path).'/../medium/medium_'.$filename;
      $isUploaded = true;
	  ...
	  if(RoxyFile::IsImage($filename)){
        RoxyImage::Resize($filePath, $fileThumbPath, 122, 91);
      }
	  if(RoxyFile::IsImage($filename)){
        RoxyImage::Resize($filePath, $fileMediumPath, 640, 480);
      }
    }
    if($errors && $errorsExt)
